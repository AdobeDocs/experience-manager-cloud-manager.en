...
To get to know Cloud Manager for AMS (Adobe Managed Services) and how it enables organizations to self-manage Adobe Experience Manager in the cloud, start here.

>[!CONTEXTUALHELP]
>id="aemcloud_cloudmanager_introduction"
>title="Introduction to Cloud Manager for AMS"
>abstract="It enables organizations to self-manage Adobe Experience Manager in the cloud using a CI/CD framework. This framework helps teams expedite customizations or updates without compromising performance or security."
...
[!UICONTROL Cloud Manager] for Adobe Experience Manager gives developers the ability to create impactful customer experiences through streamlined workflows, built upon Adobe Experience Manager best practices. CI/CD pipelines optimized for Adobe Experience Manager let you merge development workflows by checking in your code, which then moves to being production-ready. During the build phase, your custom code updates are thoroughly tested against best practices to deliver reliable applications for your customers. Cloud Manager uses an open API approach and enables you to integrate with your systems without disrupting existing processes and tools.
...
* Continuous integration/continuous delivery (CI/CD) of code to reduce development cycles from months/weeks to days/hours.
...
* Autoscaling that detects the need for increased capacity and automatically provisions additional Dispatcher/publishing segments.
...
The user interface (UI) for [!UICONTROL Cloud Manager] lets you access and manage the cloud environment and the CI/CD pipeline easily for your Adobe Experience Manager applications.
...
One of the key capabilities of [!UICONTROL Cloud Manager] is the ability to use an optimized CI/CD pipeline to speed the delivery of custom code or updates such as adding new components on the website.

Through the [!UICONTROL Cloud Manager] UI, you can configure and start your CI/CD pipeline. As part of this pipeline, a thorough code scan is executed to ensure that only high-quality applications pass through to the production environment.

To learn more about configuring pipelines from [!UICONTROL Cloud Manager]'s UI, see [Configuring Production Pipelines](/help/using/production-pipelines.md) and [Configuring Non-Production Pipelines](/help/using/non-production-pipelines.md).
...
Cloud Manager offers additional, advanced features that benefit your project depending on your particular environment setup and needs. To discuss further, reach out to your Customer Success Engineer (CSE) or Adobe representative if these features are of interest to you.

### Autoscaling {#autoscaling}

When the production environment is subject to unusually high load, [!UICONTROL Cloud Manager] detects the need for additional capacity and automatically provisions additional capacity using its autoscaling feature.

In such an event, [!UICONTROL Cloud Manager] automatically triggers the autoscaling provisioning process, sends a notification of the autoscaling event, and provisions additional capacity within minutes. The additional capacity is provisioned in the production environment and same regions, matching the system specifications of the running Dispatcher/publishing nodes.
...
* A green load balancer is created as part of the setup. This load balancer never changes and is the target for your green or "test" URL.
