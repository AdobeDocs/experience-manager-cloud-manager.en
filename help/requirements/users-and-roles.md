...
Many features in [!UICONTROL Cloud Manager] require specific permissions to use. For example, only authorized users are allowed to set the key performance indicators (KPIs) for a program. These permissions are logically grouped into roles.

[!UICONTROL Cloud Manager] defines four roles for users, which govern the availability of specific features:
...
[!UICONTROL Cloud Manager] roles are managed from the Admin Console. Adding the user to a [!UICONTROL Cloud Manager] product profile provides specific role memberships.

The Admin Console is a central location for managing your Adobe entitlements across your entire organization. To learn more about the Adobe Admin Console, see [Admin Console](https://helpx.adobe.com/business/enterprise/plan-your-deployment/basic-concepts/admin-console.html).
...
1. To set up a new role for [!UICONTROL Cloud Manager], provide the information.
   ...
   * **Permission Group** - Choose a permission group for the profile (not always available).
...
1. In the **Edit products and user groups** dialog box, click ![Add icon, plus sign](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Add_18_N.svg) and select the profiles to assign to the user.
   ...
   * If the user already is assigned to the roles, the ![Add icon, plus](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Add_18_N.svg) button is an edit button (a pencil), but functions the same way.
...
Repeat the same steps to assign profiles to user groups, but select **User Groups** from the left navigation panel on the **Users** tab. Click a user group and select the **Assigned Product Profiles**, then click **Assign Product Profile** to assign profiles.
