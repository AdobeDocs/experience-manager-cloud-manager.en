...
Cloud Manager can deploy web server and Dispatcher configuration files if they are stored in the Git repository with standard AEM content packages.

To use this capability, the Maven build produces a .zip file, which contains at least two directories: `conf` and `conf.d`. This .zip file can be produced using the `maven-assembly-plugin`.

Projects generated by Cloud Manager using the built-in [project creation wizard](/help/getting-started/using-the-wizard.md) have the correct Maven project structure created automatically. This approach is recommended if you are new to Adobe Managed Services (AMS).

When you deploy to a Dispatcher instance, the directories on the instance are replaced with those directories from your Git repository. To set the appropriate environment variables in `/etc/sysconfig/httpd` correctly, collaborate with your Customer Success team because web server and Dispatcher configuration files often require environment-specific details.

## Dispatcher configuration for existing managed service customers {#steps-for-configuring-dispatcher}

To complete the initial Dispatcher configuration, do the following:

1. Obtain the current production configuration files from your Customer Success team.
1. Remove environment-specific data, such as publish renderer IPs, and replace it with variables.
1. Define required variables in key-value pairs for each target Dispatcher and add them to the [variables](https://experienceleague.adobe.com/en/docs/experience-manager-learn/ams/dispatcher/variables) folder on each instance.
1. Test the updated configurations on your staging environment.
1. Once tested, request your Customer Success team to deploy to production.
1. Commit the files to your Git repository.
1. Deploy through Cloud Manager.

>[!NOTE]
>
>Migrating Dispatcher and web server configurations to your Git repository is done during Cloud Manager onboarding, but can also be done later.

### Example {#example}

The specific file and directory structure varies based on the specifics of your project, but this example provides a clear explanation of how to structure your project to include Apache and Dispatcher configurations.

1. Create a subdirectory named `dispatcher`.

   Use any name here, but the directory name created in this step must be the same as the name used in step 1.

1. This subdirectory contains a Maven module that builds the Dispatcher .zip file using the `maven-assembly-plugin`. In the `dispatcher` directory, create a `pom.xml` file with this content, changing the `parent` reference, `artifactId`, and `name` as necessary.
   ...
   * As in Step 1, the artifactId and name can be other values. `dispatcher` is used here as an example.

1. The `maven-assembly-plugin` requires a `descriptor` to define how the .zip file is created. To create this descriptor, create a file in the `dispatcher` subdirectory named `assembly.xml` with the following content. Note that this file name is referenced on line 26 in the `pom.xml` file above.
...
1. Create a subdirectory named `src` (as referenced in the assembly descriptor above on line 11) inside the Dispatcher subdirectory to store the actual Apache and Dispatcher configurations. Within this `src` directory, create directories named `conf`, `conf.d`, `conf.dispatcher.d`, and `conf.modules.d`.

1. Populate the `conf`, `conf.d`, `conf.dispatcher.d`, and `conf.modules.d` directories with your configuration files. For example, the default configuration consists of these files and symbolic links.
...
1. Finally, in the `pom.xml` file at your project's root, add a `<module>` element to include the Dispatcher module.

   For example, if your existing module list is the following:
...
1. To test, run `mvn clean package` in the project root directory. You see lines like this in the output.
