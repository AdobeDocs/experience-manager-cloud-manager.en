...
1. This subdirectory will contain a Maven module which builds the Dispatcher .zip file using the Maven Assembly plug-in. To start this, in the `dispatcher` directory, create a `pom.xml` file with this content, changing the `parent` reference, `artifactId`, and `name` as necessary.
