...
A content set defines the content to copy. A content set includes a list of JCR paths with the mutable content to be copied. The content moves from a source environment to a target environment. This operation is performed within the same Cloud Manager program. 
...
When copying content, the source environment is the primary reference.
...
Before any content can be copied, a content set must be defined. Once defined, content sets can be reused to copy content.
...
1. (Optional) If necessary, add additional paths (up to 50) by repeating the previous two steps. Otherwise, continue to the next step.

   ![Add paths to content set](/help/assets/add-content-set-paths.png)

1. (Optional) To refine your content set, you can optionally specify sub-paths within an included content path that should be excluded.

   1. To the right of an included content path that you want to exclude, click ![Folder delete icon](https://spectrum.adobe.com/static/icons/workflow_18/Smock_FolderDelete_18_N.svg).
   ...
   1. Repeat steps i, ii, and iii to add more excluded paths; there is no limit. Otherwise, continue to the next step.
...
To reveal the excluded sub-paths, expand the configured paths.
...
An environment can be unavailable for selection if any of the following conditions apply:
...
1. (Optional) Do any of the following:
   ...
   1. To *retain* the excluded paths in the destination environment, check **`Do not delete excluded paths from destination`**. This setting keeps the excluded paths specified in the content set intact.
   1. To *remove* the excluded paths in the destination environment, uncheck **`Do not delete excluded paths from destination`**. This setting deletes the excluded paths specified in the content set.
...
* Content copy can only be performed within the same tier. Specifically, this means author-to-author or publish-to-publish.
* Cross-program and cross-region content copy is not possible.
* Content copy for cloud-data-store-based topology can only be performed when the source and destination environments are on the same cloud provider and in the same region.
...
* Content copy duplicates assets and Dynamic Media metadata from the higher environment to the selected lower environment. Copied assets then need to be reprocessed using the [DAM process assets workflow](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/using/assets-workflow) on the lower environment. This reprocessing is required to use the respective Dynamic Media configuration.
* [Dynamic Media configurations with asset sizes greater than 2 GB enabled](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/dynamic/config-dms7#optional-config-dms7-assets-larger-than-2gb) are not supported.
